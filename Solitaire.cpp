...
Solitaire() {
        ...
        cout << "\n********************************************************************************************************************************************************";
        cout << "\nOVERALL";
        cout << "\nAVERAGE WINS: " << (int) overall_Count/NumberOfSimulations;
        cout << "\nAVERAGE (Mean) PROBABILITY: " << overall_Probability/NumberOfSimulations << " (" << overall_Probability*(100/NumberOfSimulations) << "%)";
        cout << "\nAVERAGE (Mean) EXECUTION TIME PER SIMULATION: " << overall_Time/NumberOfSimulations << " seconds";

        double prob_sd = 0;
        double ex_sd = 0;
        for(int j = 0;j<probabilities->size();j++){
            prob_sd += pow(probabilities->at(j)-(overall_Probability/100),2);
            ex_sd += pow(execution->at(j)-(overall_Time/100),2);
        }

        prob_sd = sqrt(prob_sd/100);
        ex_sd = sqrt(ex_sd/100);
